...
// Configurazione
const CONFIG = {
  zonelessUrl: 'http://localhost:4202', // URL app Angular Zoneless + Signals
 ...
 * Calcola statistiche aggregate
 */
function calculateStats2(results) {
  const swapDurations = results.map(r => r.swapMetrics.swapDuration);
  const domNodeCounts = results.map(r => r.domMetrics.domNodes);
  ...
  return {
    swapDuration: {
      avg: avg(swapDurations).toFixed(2),
      min: min(swapDurations).toFixed(2),
      max: max(swapDurations).toFixed(2),
      median: median(swapDurations).toFixed(2)
    },
    domNodes: {
      avg: avg(domNodeCounts).toFixed(0),
      min: min(domNodeCounts),
...
function generateReport(zonelessSwap, zonejsSwap, zonelessAppend, zonejsAppend, zonelessLH, zonejsLH) {
  ...
  // Aggiungi append se fornito
  if (zonelessAppend && zonejsAppend) {
    ...
    report.results.zoneless.append = {
      puppeteer: zonelessAppendStats,
      rawData: zonelessAppend
    };
    
    report.results.zonejs.append = {
  ...
  // Aggiungi lighthouse se fornito
  if (zonelessLH && zonejsLH) {
    report.results.zoneless.lighthouse = zonelessLH;
    report.results.zonejs.lighthouse = zonejsLH;
    report.comparison.lighthouseScoreDiff = (zonelessLH.performanceScore - zonejsLH.performanceScore).toFixed(2);
  }
 ...
 */
function generateReport2(zonelessResults, zonejsResults, 
  zonelessAppend, zonejsAppend,zonelessLH, zonejsLH) {
  const zonelessStats = calculateStats(zonelessResults);
  const zonejsStats = calculateStats(zonejsResults);

  const improvementPercent = (
    ((parseFloat(zonejsStats.swapDuration.avg) - parseFloat(zonelessStats.swapDuration.avg)) 
    / parseFloat(zonejsStats.swapDuration.avg)) * 100
  ).toFixed(2);
  ...
  return {
    ...
    results: {
      zoneless: {
        puppeteer: zonelessStats,
        lighthouse: zonelessLH,
        rawData: zonelessResults
      },
      zonejs: {
        puppeteer: zonejsStats,
        lighthouse: zonejsLH,
        rawData: zonejsResults
      }
    },
    comparison: {
      swapDurationImprovement: `${improvementPercent}%`,
      winner: improvementPercent > 0 ? 'Zoneless (Signals)' : 'ZoneJS',
...
function printReport(report) {
  ...
  // Mostra append solo se disponibile
  if (report.results.zoneless.append) {
  ...
  // Mostra lighthouse solo se disponibile
  if (report.results.zoneless.lighthouse) {
  ...
  // Mostra append solo se disponibile
  if (report.results.zonejs.append) {
    console.log('  📌 Append Async Duration (1000→2000 rows interleaved):');
  ...
  // Mostra lighthouse solo se disponibile
  if (report.results.zonejs.lighthouse) {
  ...
  if (report.comparison.lighthouseScoreDiff) {
 ...
 */
function printReportOld(report) {
  console.log('\n' + '='.repeat(80));
  console.log('📊 PERFORMANCE TEST RESULTS - SWAP OPERATION');
  console.log('='.repeat(80));
  
  console.log('\n🔵 ANGULAR ZONELESS (SIGNALS)');
  console.log('  Swap Duration:');
  console.log(`    Average: ${report.results.zoneless.puppeteer.swapDuration.avg}ms`);
  console.log(`    Median:  ${report.results.zoneless.puppeteer.swapDuration.median}ms`);
  console.log(`    Min:     ${report.results.zoneless.puppeteer.swapDuration.min}ms`);
  console.log(`    Max:     ${report.results.zoneless.puppeteer.swapDuration.max}ms`);
  console.log(`    Final Rows: ${report.results.zoneless.append.puppeteer.totalRows.avg}`);
  ...
  console.log('\n🟠 ANGULAR ZONEJS (TRADITIONAL)');
  console.log('  Swap Duration:');
  console.log(`    Average: ${report.results.zonejs.puppeteer.swapDuration.avg}ms`);
  console.log(`    Median:  ${report.results.zonejs.puppeteer.swapDuration.median}ms`);
  console.log(`    Min:     ${report.results.zonejs.puppeteer.swapDuration.min}ms`);
  console.log(`    Max:     ${report.results.zonejs.puppeteer.swapDuration.max}ms`);
  console.log(`  Lighthouse Score: ${report.results.zonejs.lighthouse.performanceScore.toFixed(0)}/100`);
  console.log(`  FCP: ${report.results.zonejs.lighthouse.metrics.firstContentfulPaint.toFixed(0)}ms`);
  console.log(`  LCP: ${report.results.zonejs.lighthouse.metrics.largestContentfulPaint.toFixed(0)}ms`);
  ...
  console.log(`  Swap Performance Improvement: ${report.comparison.swapDurationImprovement}`);
  console.log(`  Lighthouse Score Difference: ${report.comparison.lighthouseScoreDiff} points`);
...
async function main() {
  try {
    ...
    // Test Zoneless (Signals)
    const zonelessResults = await measureSwapPerformance(
      CONFIG.zonelessUrl, 
      'Angular Zoneless (Signals)'
    );
    ...
    // Test ZoneJS
    const zonejsResults = await measureSwapPerformance(
      CONFIG.zonejsUrl, 
      'Angular ZoneJS (Traditional)'
    );
...
async function generateHTMLReport(report) {
  const hasAppend = report.results.zoneless.append && report.results.zonejs.append;
  const hasLighthouse = report.results.zoneless.lighthouse && report.results.zonejs.lighthouse;
...
<head>
  ...
  <style>
    ...
    .grid-2 {
      display: grid;
      grid-template-columns: ${hasAppend ? '1fr 1fr' : '1fr'};
      gap: 20px;
    }
    .chart-container {
      position: relative;
...
<body>
  <div class="header">
    ...
    <p>Tests: Swap Operation${hasAppend ? ' + Append Async (Interleaved)' : ''}</p>
