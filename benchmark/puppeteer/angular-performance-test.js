...
async function main() {
  try {
    ...
    const runAppendTests = process.argv.includes('--with-append');
    const runLighthouse = process.argv.includes('--with-lighthouse');
    
    if (runAppendTests) {
      console.log(`   Tests: Swap + Append Async\n`);
    } else {
      console.log(`   Tests: Swap only (use --with-append to include append tests)\n`);
    }
    ...
    // Test Zoneless (Signals) - APPEND (opzionale)
    let zonelessAppend = null;
    if (runAppendTests) {
      zonelessAppend = await measureAppendAsyncPerformance(
        CONFIG.zonelessUrl, 
        'Angular Zoneless (Signals) - Append'
      );
    }
    
    // Lighthouse Zoneless (opzionale)
    let zonelessLH = null;
    if (runLighthouse) {
      zonelessLH = await runLighthouseAudit(
        CONFIG.zonelessUrl, 
        'Angular Zoneless (Signals)'
      );
    }
    ...
    // Test ZoneJS - APPEND (opzionale)
    let zonejsAppend = null;
    if (runAppendTests) {
      zonejsAppend = await measureAppendAsyncPerformance(
        CONFIG.zonejsUrl, 
        'Angular ZoneJS (Traditional) - Append'
      );
    }
    
    // Lighthouse ZoneJS (opzionale)
    let zonejsLH = null;
    if (runLighthouse) {
      zonejsLH = await runLighthouseAudit(
        CONFIG.zonejsUrl, 
        'Angular ZoneJS (Traditional)'
      );
    }
...
//   await fs.writeFile('performance-report.html', html2);
}
